...
-(void)addDetailViews{
    ...
    self.chartView = [[UIScrollView alloc]initWithFrame:CGRectMake(_leftTitleW + 5, 0, self.bounds.size.width-_leftTitleW, self.bounds.size.height)];
    self.chartView.backgroundColor = [UIColor clearColor];
    
    [self addSubview:self.lineView];
    [self addSubview:self.chartView];
    
    [self.chartView addSubview:self.curveView];
...
- (void)resetUIAndData {
    ...
    if (_dataSource.count > 7) {
        ...
        _chartView.frame = CGRectMake(0, 0, self.bounds.size.width, 0);
...
-(UIView *)curveView{
    if (!_curveView) {
        _curveView = [[UIView alloc]initWithFrame:CGRectMake(5, 40, self.chartView.bounds.size.width-5, self.chartView.bounds.size.height)];
    }
    return _curveView;
}
...
-(UIView *)lineView{
    if (!_lineView) {
        _lineView = [[UIView alloc]initWithFrame:CGRectMake(CGRectGetMinX(_chartView.frame) , 0 + 40, self.chartView.bounds.size.width - 5, self.chartView.bounds.size.height-80)];
    }
    return _lineView;
}
...
- (UILabel *)titleLForX {
    if (!_titleLForX) {
        _titleLForX = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.lineView.frame) - 90, CGRectGetMaxY(self.chartView.frame) - _yMargin * 2 - 25, 60, 20)];
...
-(void)setDataSource:(NSArray *)dataSource{
    ...
    _dataSource = dataSource;
   // [self.chartView setContentOffset:CGPointZero animated:NO];
    [self resetLeftDataSource];
    [self resetUIAndData];
}
